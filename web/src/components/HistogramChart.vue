...
const sig = computed(() => `${props.run.id}:${props.run.updated_at}~${props.metricKey}`)
...
async function update() {
  ...
  const yLabels = Array.from({ length: nBins }, (_, j) =>
    edges[j] !== undefined && edges[j + 1] !== undefined ? fmtNum((edges[j] + edges[j + 1]) / 2) : String(j),
  )
  ...
  chart.setOption(
    {
      ...
      visualMap: {
        ...
        inRange: { color: ['#15151b', '#243056', '#3a6ea8', '#5fb3f5', '#9adb5e', '#facc5f'] },
      },
      series: [
        {
          type: 'heatmap',
          ...
          emphasis: { itemStyle: { borderColor: 'rgba(255,255,255,0.4)', borderWidth: 1 } },
        },
      ],
    },
    { notMerge: true },
