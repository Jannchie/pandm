...
def test_sdk_local_roundtrip(data_dir):
    run = pandm.init(
        project="proj", name="test-run", config={"lr": 0.1}, directory=data_dir
    )
    for step in range(20):
        run.log({"loss": 1.0 / (step + 1), "acc": step / 20}, step=step)
    run.log_image(
        "samples", Image.new("RGB", (16, 16), (255, 0, 0)), step=19, caption="hi"
    )
    run.finish()
...
def test_sdk_tags_and_group_roundtrip(data_dir):
    run = pandm.init(
        ...
        name="tagged",
        directory=data_dir,
        tags=["baseline", "lr-sweep", "baseline", " trim ", ""],
        group="exp-42",
    )
    run.finish()
...
def test_server_read_api(data_dir):
    run = pandm.init(
        project="proj", name="api-run", config={"bs": 32}, directory=data_dir
    )
...
def test_ingest_api_with_key(data_dir):
    ...
    # without key -> 401
    assert (
        client.post("/api/runs", json={"project": "p", "name": "n"}).status_code == 401
    )

    headers = {"x-api-key": "sekrit"}
    run_id = client.post(
        "/api/runs",
        json={"id": "abc12345", "project": "p", "name": "n", "config": {"a": 1}},
        headers=headers,
    ).json()["id"]
    ...
    rows = {
        "rows": [
            {"key": "loss", "step": i, "value": float(i), "ts": 1.0 + i}
            for i in range(5)
        ]
    }
    assert (
        client.post(f"/api/runs/{run_id}/metrics", json=rows, headers=headers).json()[
            "inserted"
        ]
        == 5
    )
    ...
    assert (
        client.post(
            f"/api/runs/{run_id}/finish", json={"status": "finished"}, headers=headers
        ).status_code
        == 200
    )
...
def test_delete_project_removes_all_runs(data_dir):
    ...
    for i in range(3):
        client.post(
            "/api/runs",
            json={"id": f"r{i}", "project": "doomed", "name": f"n{i}"},
            headers=headers,
        )
    client.post(
        "/api/runs",
        json={"id": "keep", "project": "other", "name": "n"},
        headers=headers,
    )
...
def test_stale_running_run_reported_crashed(data_dir):
    ...
    with store._lock:
        store._db.execute(
            "UPDATE runs SET updated_at = ? WHERE id = 'r1'", (time.time() - 120,)
        )
        store._db.commit()
...
def test_cli_show_and_export(data_dir):
    ...
    run = pandm.init(
        project="p", name="cli-run", config={"lr": 0.1}, directory=data_dir
    )
    for step in range(5):
    ...
    one_key = runner.invoke(
        cli_app, ["export", run.id, "-k", "loss", "--json", "--dir", str(data_dir)]
    )
...
def test_progress_ingest_api(data_dir):
    ...
    headers = {"x-api-key": "k"}
    client.post(
        "/api/runs", json={"id": "p1", "project": "p", "name": "n"}, headers=headers
    )
...
def test_resume_continues_run(data_dir):
    r1 = pandm.init(
        project="p", id="run-aaaa", config={"lr": 0.1}, directory=data_dir, remote=False
    )
    for s in range(3):
    ...
    r2 = pandm.init(
        project="p", id="run-aaaa", resume=True, directory=data_dir, remote=False
    )
...
def test_resume_guards(data_dir):
    ...
    with pytest.raises(ValueError):
        pandm.init(
            project="p", id="ghost", resume="must", directory=data_dir, remote=False
        )
    # resume=True on a missing id just starts fresh
    r = pandm.init(
        project="p", id="brand-new", resume=True, directory=data_dir, remote=False
    )
...
def test_summary_author_scalars(data_dir):
    ...
    assert (
        r["stats"]["loss"]["last"] == 1.0
    )  # latest metric lives in stats, not summary
...
def test_finish_ingests_summary(data_dir):
    ...
    headers = {"x-api-key": "k"}
    client.post(
        "/api/runs", json={"id": "s1", "project": "p", "name": "n"}, headers=headers
    )
...
def test_define_metric_local(data_dir):
    ...
    r = _get_run(LocalStore(data_dir), run.id)
    assert (
        r["description"] == "MNIST 基线扫参"
    )  # run subtitle, written in the user's language
    meta = r["metric_meta"]
    ...
    assert meta["acc"] == {
        "min": 0.0,
        "max": 1.0,
    }  # unit="percent" not set -> just the fixed range
...
def test_define_metric_panel_band_kind(data_dir):
    ...
    run.define_metric(
        "eval/return", band={"lo": "eval/return_p05", "hi": "eval/return_p95"}
    )
    for s in range(4):
...
def test_define_metric_rejects_bad_args(data_dir):
    ...
    assert (
        "x" not in meta
    )  # only rejected/no-op calls happened, nothing reached the store
...
def test_finish_ingests_metric_meta(data_dir):
    ...
    headers = {"x-api-key": "k"}
    client.post(
        "/api/runs", json={"id": "m1", "project": "p", "name": "n"}, headers=headers
    )
...
def test_log_histogram_local(data_dir):
    ...
    for step in range(3):
        run.log_histogram(
            "dist/reward", np.random.randn(500) + step, step=step, bins=20
        )
    run.log_histogram(
        "adv", ([1, 2, 3], [0.0, 1.0, 2.0, 3.0]), step=3
    )  # precomputed (counts, edges)
...
def test_histogram_seq_dedup(data_dir):
    ...
    n = store.log_histograms_seq(
        "r", [("k", 0, [0.0, 1.0], [2], 1.0, 1), ("k", 1, [0.0, 1.0], [3], 2.0, 2)]
    )
    assert n == 2
    # a replay (seq ≤ watermark) is dropped; only the fresh seq=3 lands
    n2 = store.log_histograms_seq(
        "r", [("k", 1, [0.0, 1.0], [3], 2.0, 2), ("k", 2, [0.0, 1.0], [5], 3.0, 3)]
    )
    assert n2 == 1
...
def test_histogram_ingest_and_read_api(data_dir):
    ...
    headers = {"x-api-key": "k"}
    client.post(
        "/api/runs", json={"id": "h1", "project": "p", "name": "n"}, headers=headers
    )
    ...
    assert (
        client.post("/api/runs/h1/histograms", json=rows, headers=headers).json()[
            "inserted"
        ]
        == 2
    )
...
def test_init_prints_run_banner(data_dir, capsys):
    ...
    assert (
        run.id in err and "banner-run" in err
    )  # id surfaced for `pandm delete`, name to recognize
...
def test_meta_endpoint_merges_live(data_dir):
    ...
    headers = {"x-api-key": "k"}
    client.post(
        "/api/runs", json={"id": "mt1", "project": "p", "name": "n"}, headers=headers
    )
