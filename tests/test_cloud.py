...
def test_run_delete_dual(local_dir, server):
    ...
    assert (
        client.get(f"/api/runs/{run.id}").status_code == 404
    )  # gone (or never synced) on the server
    assert (
        local_dir and LocalStore(local_dir).runs_needing_sync() == []
    )  # no sync-queue residue
...
def test_offline_then_backfill(local_dir, server):
    ...
    local = LocalStore(local_dir)
    assert (
        len(local.metric_series(run.id, "loss")["steps"]) == 3
    )  # local-first: nothing lost
    assert (
        client.get(f"/api/runs/{run.id}").status_code == 404
...
def test_finish_only_after_tail_synced(local_dir, server):
    ...
    if remote_state.status_code == 200:  # whatever made it before the cut
        assert (
            remote_state.json()["status"] == "running"
        )  # finish never outran the data
...
def test_deadline_short_circuits_sync(local_dir, server):
    ...
    with remote.deadline(0.0):  # budget already spent before any request
        assert (
            remote.run_exists("rdl00001") is False
        )  # read short-circuits, never blocks
        assert pump_run(local, remote, "rdl00001") is False  # nothing pushed
    assert (
        client.get("/api/runs/rdl00001/metrics/loss").json()["values"] == []
    )  # tail untouched
...
def test_resolve_remote_matrix(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "PANDM_REMOTE", "https://direct"
    )  # env wins, remote-only semantics
    ...
    assert credentials.resolve_remote("https://arg") == (
        "remote_only",
        "https://arg",
        "envkey",
    )
...
def mu(server_dir, monkeypatch):
    ...
    def session_for(user):
        return _sign(
            load_secret(server_dir), {"uid": user["id"], "exp": time.time() + 3600}
        )
...
def test_user_isolation(mu):
    ...
    run_id = client.post(
        "/api/runs", json={"id": "arun1234", "project": "p", "name": "a"}, headers=a_key
    ).json()["id"]
    rows = {"rows": [{"key": "loss", "step": 0, "value": 1.0, "ts": 1.0}]}
    assert (
        client.post(f"/api/runs/{run_id}/metrics", json=rows, headers=a_key).status_code
        == 200
    )
    ...
    assert (
        client.get(f"/api/runs/{run_id}/metrics/loss", headers=b_key).status_code == 404
    )
    assert (
        client.post(f"/api/runs/{run_id}/metrics", json=rows, headers=b_key).status_code
        == 404
    )
...
def test_session_cookie_and_key_rotation(mu):
    ...
    assert (
        client.get("/api/me", headers={"x-api-key": alice["api_key"]}).status_code
        == 401
    )
...
def test_oauth_callback(mu, monkeypatch):
    ...
    def fake_post(url, **kwargs):
        ...
        return httpx.Response(
            200, json={"access_token": "gh-token"}, request=httpx.Request("POST", url)
        )
    ...
    assert (
        client.get(
            "/api/auth/callback?code=c&state=WRONG", follow_redirects=False
        ).status_code
        == 403
    )
...
def test_device_flow(mu):
    ...
    assert (
        client.post("/api/cli/approve", json={"code": start["user_code"]}).status_code
        == 401
    )
    ...
    client.cookies.clear()
    poll = client.post(
        "/api/cli/poll", json={"device_token": start["device_token"]}
    ).json()
...
def test_dual_write_against_multi_user_server(local_dir, mu):
    ...
    assert (
        client.get(
            f"/api/runs/{run.id}", headers={"x-api-key": alice["api_key"]}
        ).status_code
        == 200
    )
    assert (
        client.get(
            f"/api/runs/{run.id}", headers={"x-api-key": bob["api_key"]}
        ).status_code
        == 404
    )
...
def test_cli_delete_local_only_when_logged_out(tmp_path, monkeypatch):
    ...
    result = CliRunner().invoke(
        cli_app, ["delete", "run00001", "--dir", str(data_dir), "--yes"]
    )
...
def test_cli_delete_also_deletes_cloud_copy(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "httpx.delete",
        lambda url, **kw: server_client.delete(
            url.removeprefix(SERVER_URL), headers=kw.get("headers")
        ),
    )
...
def test_cli_delete_local_only_flag_keeps_cloud_copy(tmp_path, monkeypatch):
    ...
    credentials.save(SERVER_URL, "sekrit")
    monkeypatch.setattr(
        "httpx.delete", lambda *a, **kw: pytest.fail("must not touch the server")
    )
    ...
    result = CliRunner().invoke(
        cli_app, ["delete", "run00001", "--dir", str(data_dir), "--yes", "--local-only"]
    )
    assert result.exit_code == 0
    assert LocalStore(data_dir).get_run("run00001") is None
...
def test_cli_delete_missing_everywhere_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path / "xdg"))
    result = CliRunner().invoke(
        cli_app, ["delete", "nope", "--dir", str(tmp_path / ".pandm"), "--yes"]
    )
...
def test_device_login_roundtrip(mu, fresh_config, monkeypatch):
    ...
    saved = credentials.device_login(
        SERVER_URL, open_browser=False, echo=lambda _m: None, poll_interval=0.0
    )
...
def test_device_login_unreachable_returns_none(fresh_config, monkeypatch):
    ...
    assert (
        credentials.device_login("http://nope", echo=lambda m: None, poll_interval=0.0)
        is None
    )
...
def test_cli_login_signs_in_and_opts_out(mu, fresh_config, monkeypatch):
    ...
    monkeypatch.setattr(
        creds_mod,
        "device_login",
        lambda *a, **k: real(*a, **{**k, "poll_interval": 0.0}),
    )
...
def test_init_offer_keep_local_opts_out(fresh_config, monkeypatch):
    ...
    assert sdk._maybe_offer_login(Remote("local", None, None), None, None) == Remote(
        "local", None, None
    )
    ...
    monkeypatch.setattr(sdk, "_login_offered", False)
    monkeypatch.setattr(
        "builtins.input", lambda *a: pytest.fail("must not prompt after opt-out")
    )
    assert sdk._maybe_offer_login(Remote("local", None, None), None, None) == Remote(
        "local", None, None
    )
...
def test_init_offer_suppressed_by_env(fresh_config, monkeypatch):
    ...
    monkeypatch.setattr(sdk, "_is_interactive", lambda: True)
    monkeypatch.setattr(
        "builtins.input", lambda *a: pytest.fail("must not prompt when suppressed")
    )
    ...
    assert sdk._maybe_offer_login(Remote("local", None, None), False, None) == Remote(
        "local", None, None
    )
...
def test_remote_backend_resume(server):
    ...
    client.post(
        "/api/runs/rr000001/metrics",
        json={"rows": [{"key": "loss", "step": 7, "value": 1.0, "ts": 1.0}]},
    )
